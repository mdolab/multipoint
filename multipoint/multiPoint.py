    ...
    def fun_obj(self, x):
            ...
            if self.setFlags[key]:
                ...
                for func in self.pSet[key].functionals:
                    ...
                    if self.pSet[key].functionals[func]["unique"]:
                        ...
                        for i in range(self.pSet[key].nMembers):
                            if self.pSet[key].groupID == i and self.pSet[key].comm.rank == 0:
                                val[i] = self.pSet[key].gcomm.bcast(res[func], root=self.pSet[key].cumGroups[i])
                            else:
                                val[i] = self.pSet[key].gcomm.bcast(None, root=self.pSet[key].cumGroups[i])
                            # end if
                        # end for
                ...
                # pSet gcomm:
                setFunctionals["fail"] = self.pSet[key].gcomm.allreduce(res["fail"], op=MPI.LOR)
            # end if
        # end for
        ...
        for key in self.pSet.keys():
            if self.setFlags[key] and self.pSet[key].gcomm.rank == 0:
                tmp = self.gcomm.bcast(setFunctionals, root=self.cumSets[self.pSet[key].setID])
            else:
                tmp = self.gcomm.bcast(None, root=self.cumSets[self.pSet[key].setID])
            # end if
        ...
        # MP gcomm
        functionals["fail"] = self.gcomm.allreduce(setFunctionals["fail"], op=MPI.LOR)
    ...
    def sens(self, x, f_obj, f_con):
        ...
        for key in self.pSet.keys():
            if self.setFlags[key]:
                ...
                # Special case for fail. Simply logical OR over entire
                # pSet gcomm:
                setDerivatives["fail"] = self.pSet[key].gcomm.allreduce(res["fail"], op=MPI.LOR)
            # end if
        # end for
        ...
        for key in self.pSet.keys():
            if self.setFlags[key] and self.pSet[key].gcomm.rank == 0:
                tmp = self.gcomm.bcast(setDerivatives, root=self.cumSets[self.pSet[key].setID])
            else:
                tmp = self.gcomm.bcast(None, root=self.cumSets[self.pSet[key].setID])
            # end if
        ...
        # Special case for or. Simply logical OR over entire
        # MP gcomm
        derivatives["fail"] = self.gcomm.allreduce(setDerivatives["fail"], op=MPI.LOR)
